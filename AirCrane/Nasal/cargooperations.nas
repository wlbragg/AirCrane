...
setlistener("/sim/signals/fdm-initialized", func (n) {
  ...
  #tower-sec-one   -14.053 m = 46.14       12.81             16.49 new 8.52
  #tower-sec-two   -14.129 m = 46.35       12.94             16.49 new 8.52
  #tower-sec-three -14.323 m = 46.99       13.06             16.49 new 8.52
  #tower-sec-four  -14.416 m = 47.29       13.24             16.49 new 8.52
  #tower-sec-five  -14.436 m = 47.36       13.22             16.49 new 8.52
  #tower-sec-six   -14.435 m = 47.35       13.22             16.49 new 8.52
  #tower-sec-seven -14.469 m = 47.47       13.25             16.49 new 8.52
  #tower-radio    -162.331 m =532.58      173.63             16.49 new 8.52
  ...
  cargo17 = place_model("17", 16, "tower-sec-one",    model_path,  0,12.81, 1400,12.81, 8.52, lat, lon, alt-90, 46.10, 0, 0, 0);
  cargo18 = place_model("18", 17, "tower-sec-two",    model_path, 16,12.94, 1300,12.94, 8.52, lat, lon, alt-90, 46.35, 0, 0, 0);
  cargo19 = place_model("19", 18, "tower-sec-three",  model_path, 17,13.06, 1200,13.06, 8.52, lat, lon, alt-90, 46.99, 0, 0, 0);
  cargo20 = place_model("20", 19, "tower-sec-four",   model_path, 18,13.24, 1100,13.24, 8.52, lat, lon, alt-90, 47.29, 0, 0, 0);
  cargo21 = place_model("21", 20, "tower-sec-five",   model_path, 19,13.22, 1000,13.22, 8.52, lat, lon, alt-90, 47.36, 0, 0, 0);
  cargo22 = place_model("22", 21, "tower-sec-six",    model_path, 20,13.22,  950,13.22, 8.52, lat, lon, alt-90, 47.35, 0, 0, 0);
  cargo23 = place_model("23", 22, "tower-sec-seven",  model_path, 21,16.49,  900,13.25, 8.52, lat, lon, alt-90, 47.47, 0, 0, 0);
  cargo24 = place_model("24", 23, "tower-radio",      model_path,  0,  0.0,  500,173.63,8.52,lat, lon, alt-90,532.58, 0, 0, 0);
  cargo25 = place_model("25", 24, "radio-tower-sec",  model_path, 30,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
  cargo26 = place_model("26", 25, "radio-tower-sec",  model_path, 24,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
  cargo27 = place_model("27", 26, "radio-tower-sec",  model_path, 25,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
  cargo28 = place_model("28", 27, "radio-tower-sec",  model_path, 26,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
  cargo29 = place_model("29", 28, "radio-tower-sec",  model_path, 27,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
  cargo30 = place_model("30", 29, "radio-tower-sec",  model_path, 28,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
  cargo31 = place_model("31", 30, "radio-tower-sec",  model_path, 29,24.84,  500,24.84, 8.52, lat, lon, alt-90, 85.49, 0, 0, 0);
...
var cargoWeight = 0;
var cargoHarness = 0;
#cargoOffset = top of cargo
var cargoOffset = 1.21;
var haulable = 0;
var stack = 0;
          ...
          if (altNode < (hookHeight + (cargoHeight * 3.28)))
            {
			        if (string.match(cargoN.getNode("callsign").getValue(), "cargo*"))
                {
                  ...
                  #needs to be variable for either hard docked distance or long line distance
                  if (cargo_dist <= (ropeLength + 5))
                    {
  ...
  if (hooked == 1) {
    ...
    if (!longline) {
      ...
      if (longline and !cargoOnGround) {
        ...
        setprop("sim/cargo/cargoalt", -17.1 + -cargoHarness + -hitchOffset + cargoOffset);
        setprop("sim/cargo/harnessalt", -hitchOffset + -17.1);
        ...
        if (stack > 0) {
          ...
          var stackTopHeight = getprop("/models/cargo/cargo[" ~ stack ~ "]/elevation-ft") + (getprop("/models/cargo/cargo[" ~ stack ~ "]/height") * 3.28) - getprop("/models/cargo/cargo[" ~ stack ~ "]/elev-offset");
          var hdist = cargoBaseHeight - stackTopHeight;
          ...
          if (cargo_dist <= 10 and (hdist > -1 and hdist < 2)) {
